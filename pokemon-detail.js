...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // Turn the pokemonID into a number (in 10 decimal format)
    const id = parseInt(pokemonID, 10);
    
    // Return to main page if id is not valid.
...
async function loadPokemon(id) 
{
    try 
    {
        ...
        if (currentPokemonId === id) 
        {
            ...
            // Getting the pokemon description
            const flavorText = getEnglishFlavorText(pokemonSpecies);
            document.querySelector(".body3-fonts.pokemon-description").textContent = flavorText;

            // The black chevron left and right arrow
            const [leftArrow, rightArrow] = ["#leftArrow", "#rightArrow"].map((sel) => document.querySelector(sel));
            leftArrow.removeEventListener("click", navigatePokemon);
            rightArrow.removeEventListener("click", navigatePokemon);

            // Case where user tries to go to previous Pokemon when its ID is 1. (The first pokemon) 
            if (id !== 1) 
            {
            ...
            // Case where user tries to go to next Pokemon when its ID is the MAX_POKEMON
            if (id !== 151) 
            {
            ...
            // Changes the page URL whenever moving to different Pokemon via the left/right arrow.
            // Prevents the page from reloading when moving to other Pokemons  
...
function displayPokemonDetails(pokemon) 
{
    ...
    // Main image for Pokemon in the detail page
